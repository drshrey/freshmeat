...
with open('murder.csv', 'rb') as csvfile:
	...
	for row in spamreader:
		...
		newMurder.save()
'''
body_parts = ['Left Arm', 'Right Arm', 'Right Leg', 'Left Leg', 'Tail']
import random
murders = Murder.select()
for mu in murders:
	print "cur", mu.body_part_found
	mu.body_part_found = random.choice(body_parts)
	print "changed", body_parts
	mu.save()
